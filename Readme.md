Acrome Mini Robot – Simulation Setup & Usage
Manual
...
Sensör Verileri ve Monitoring
 ...
 LIDAR Veri Akışı : ros2 topic echo /scan
Odometry Verileri :ros2 topic echo /odom
Sistem Monitoring
 ROS Graph Görüntüleme : rqt_graph
 Aktif Topic'lerin Listelenmesi : ros2 topic list
Aktif Node'ların Listelenmesi : ros2 node list

Hata Ayıklama 
Log ve Hata Mesajları
ros2 topic echo /rosout
 TF (Transform) Sistem Kontrolü
TF Tree Görüntüleme:ros2 run tf2_tools view_frames
Parametre Kontrolü
...
Çözüm: rosdep install --from-paths src --ignore-src -r -y
...
ros2 launch acrome_bringup gazebo.launch.py
 ...
 Referanslar ve Faydalı Komutlar
 Temel ROS 2 Komutları
Komut	Açıklama
ros2 topic list	Aktif topic'leri listeler
ros2 node info <node_name>	Node detaylarını gösterir
ros2 interface show <msg_type>	Message yapısını gösterir
ros2 param list	Parametreleri listeler


Acrome Özel Komutları
Komut	Açıklama
source ~/acrome_ws/install/setup.bash	Çalışma alanını aktif eder
ros2 launch acrome_mini_robot gazebo_launch.py	Gazebo'yu başlatır
python3 src/acrome_mini_robot/launch/robot_controller.py	Hareketi başlatır
