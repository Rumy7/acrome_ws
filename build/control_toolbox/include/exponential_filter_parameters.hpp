#pragma message("#include \"exponential_filter_parameters.hpp\" is deprecated. Use #include <control_toolbox/exponential_filter_parameters.hpp> instead.")
...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
  ...
  public:
    ...
    : ParamListener(parameters_interface, rclcpp::get_logger("exponential_filter"), prefix) {
    ...
    rcl_interfaces::msg::SetParametersResult update(const std::vector<rclcpp::Parameter> &parameters) {
      ...
      for (const auto &param: parameters) {
        if (param.get_name() == (prefix_ + "alpha")) {
            if(auto validation_result = bounds<double>(param, 0.0, 1.0);
              !validation_result) {
                return rsl::to_parameter_result_msg(validation_result);
            }
            updated_params.alpha = param.as_double();
    ...
    void declare_params(){
      ...
      if (!parameters_interface_->has_parameter(prefix_ + "alpha")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "Exponential decay factor";
          ...
          parameters_interface_->declare_parameter(prefix_ + "alpha", parameter, descriptor);
      }
      // get parameters and fill struct fields
      rclcpp::Parameter param;
      param = parameters_interface_->get_parameter(prefix_ + "alpha");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = bounds<double>(param, 0.0, 1.0);
        !validation_result) {
          throw rclcpp::exceptions::InvalidParameterValueException(fmt::format("Invalid value set during initialization for parameter 'alpha': {}", validation_result.error()));
      }
      updated_params.alpha = param.as_double();
